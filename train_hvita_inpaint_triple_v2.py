...
def main(args):
    ...
    netG = ResnetGenerator128_hvita_triple(num_classes=num_classes, output_dim=3, pred_classes=pred_classes).to(device)
    netD = HVITADiscriminator128().to(device)
    netOD = HVITADiscriminator64_triple(num_classes=num_classes, pred_classes=pred_classes).to(device)
    ...
    for epoch in range(args.start_epoch, args.total_epoch):
        ...
        for idx, data in enumerate(tqdm(dataloader)):
            ...
            content = {'image_contents': masked_images, 'mask': mask, 'label': label, 'bbox': selected_bbox, 'triples': triples, 'z': z, 'randomly_selected': randomly_selected.unsqueeze(0).expand(b, -1)}
            ...
            # update OD network
            netOD.zero_grad()
            d_out_o_real = netOD(obj_images, label, triples, randomly_selected.unsqueeze(0).expand(b, -1))
            d_loss_o_real = torch.nn.ReLU()(1.0 - d_out_o_real).mean()
            d_out_o_fake = netOD(fake_obj_images.detach(), label, triples, randomly_selected.unsqueeze(0).expand(b, -1))
            ...
            if (idx % 1) == 0:
                ...
                g_out_o_fake = netOD(fake_obj_images, label, triples, randomly_selected.unsqueeze(0).expand(b, -1))
                ...
                g_loss = g_loss_fake + pixel_loss + g_loss_o_fake + o_pixel_loss
                g_loss.backward()
                g_optimizer.step()
            ...
            if idx % (total_idx/4 + 1) == 0:
                ...
                writer.add_scalars("D_loss_real", {"real": d_loss_real.item(),
                                                    'real_obj': d_loss_o_real.item(),
                                                       "loss": d_loss.item(),
                                                       "od_loss": od_loss.item()}, epoch * len(dataloader) + idx + 1)
