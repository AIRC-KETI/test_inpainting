...
class ResnetDiscriminator128(nn.Module):
    def __init__(self, num_classes=0, input_dim=3, ch=64):
        ...
        self.block1 = conv2d(input_dim, ch, 5, 2, 2)
        self.block2 = conv2d(ch, ch * 2, 5, 2, 2)
        self.block3 = conv2d(ch * 2, ch * 4, 5, 2, 2)
        self.block4 = conv2d(ch * 4, ch * 4, 5, 2, 2)
        self.block5 = conv2d(ch * 4, ch * 4, 5, 2, 2)
        # self.block6 = conv2d(ch * 4, ch * 4, 5, 2, 2)
        ...
        self.block_obj3 = conv2d(ch * 2, ch * 4, 5, 1, 1)
        self.block_obj4 = conv2d(ch * 4, ch * 4, 5, 1, 1)
        self.block_obj5 = conv2d(ch * 4, ch * 4, 5, 2, 2)
        self.l_obj = nn.utils.spectral_norm(nn.Linear(ch * 4, 1))
        self.l_y = nn.utils.spectral_norm(nn.Embedding(num_classes, ch * 4))
    ...
    def forward(self, x, y=None, bbox=None):
        ...
        features.append(x)
        # 8x8
        out_im = x = self.block5(x)
        features.append(x)
        # 4x4
        # out_im = x = self.block6(x)
        # features.append(x)
        ...
        return out_im, out_obj, features
...
class CombineDiscriminator128(nn.Module):
    def __init__(self, num_classes=81):
        ...
        self.obD = ResnetDiscriminator128(num_classes=num_classes, input_dim=4)
    ...
    def forward(self, images, bbox, label, mask=None):
        ...
        bbox = bbox[idx]
        label = label[idx]
        d_out_img, d_out_obj, features = self.obD(images, label, bbox)
        return d_out_img, d_out_obj, features
...
class MultiMaskDiscriminator(nn.Module):
    def __init__(self, input_dim=34, ch=64):
        super(MultiMaskDiscriminator, self).__init__()
        self.block1 = conv2d(input_dim, ch, 5, 2, 2)
        self.block2 = conv2d(ch, ch * 2, 5, 2, 2)
        self.block3 = conv2d(ch * 2, ch * 4, 5, 2, 2)
        self.block4 = conv2d(ch * 4, ch * 4, 5, 2, 2)
