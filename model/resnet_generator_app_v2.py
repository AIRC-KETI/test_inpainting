...
def box_attention(query, key, value, box_relation_embds_matrix, mask=None, dropout=None):
    ...
    output = torch.matmul(w_mn, w_v)  # [B, len_q, len_k], [B, len_v, d_k]
    # print('[final matmul]')
    # print(output)
    return output, w_mn
...
class BoxMultiHeadedAttention(nn.Module):
    ...
    def forward(self, input_query, input_key, input_value, input_box, mask=None):
        ...
        # print('[*] origin')
        # print(input_query, input_key, input_value)
...
class SemanticTripleAttention(nn.Module):
    ''' Scaled Dot-Product Attention '''
    ...
    def forward(self, q, k, v, mask):
        ...
        # attn = torch.bmm(q, k.transpose(1, 2))  # [32, 8, 308]  # [32, 8, 308]
        # print(attn.shape)
        attn = q + k
        if mask is not None:
            mask = mask.unsqueeze(1).expand(mask.size(0), q.size(1), mask.size(1))
        ...
        # attn = self.softmax(attn)
        # attn = self.dropout(attn)
        # output = torch.bmm(attn.transpose(1, 2), v)
        output = attn * v
        return output, attn, log_attn
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        # print('[*] qkv size: {}, {}, {}'.format(q.size(), k.size(), v.size()))  #[16, 16, 512], [16, 31, 512], [16, 31, 512]
        sz_b0, len_q, _ = q.size()
        sz_b, len_k, _ = k.size()
        sz_b, len_v, _ = v.size()

        residual = q  # print(residual.size()) [B, h*w, ch]
        ...
        v = v.permute(2, 0, 1, 3).contiguous().view(-1, len_v, d_v)  # (n*b) x lv x dv  # print(q.size(), k.size(), v.size())  [128, 16, 512], [128, 31, 512], [128, 31, 512]

        output, attn, log_attn = self.attention(q, k, v, mask=mask)  # print(output.size())  [128, 16, 512]
        output = output.view(n_head, sz_b, len_q, d_v)  # print(output.size())  [8, 16, 16, 512]
        output = output.permute(1, 2, 0, 3).contiguous().view(sz_b0, len_q, -1)  # b x lq x (n*dv)  # print(output.size())  [16, 16, 4096]
...
class SemanticTripleAttentionModule(nn.Module):
    ''' Semantic Triple Attention module '''
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        ...
        self.attention = SemanticTripleAttention(temperature=np.power(d_k, 0.5))
        self.layer_norm = nn.LayerNorm(d_model)
        self.layer_norm0 = nn.LayerNorm(d_model)
...
class ResnetGenerator128_inpaint(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3):
        super(ResnetGenerator128_inpaint, self).__init__()
        self.num_classes = num_classes
        ...
        self.res1 = ResBlock(ch * 16 + 3, ch * 16, upsample=True, num_w=num_w)
        self.res2 = ResBlock(ch * 16 + 3, ch * 8, upsample=True, num_w=num_w)
        self.res3 = ResBlock(ch * 8 + 3, ch * 4, upsample=True, num_w=num_w)
        self.res4 = ResBlock(ch * 4 + 3, ch * 2, upsample=True, num_w=num_w, psp_module=True)
        self.res5 = ResBlock(ch * 2 + 3, ch * 1, upsample=True, num_w=num_w, predict_mask=False)
        self.final = nn.Sequential(BatchNorm(ch),
                                   nn.ReLU(),
                                   conv2d(ch, output_dim, 3, 1, 1),
                                   nn.Tanh())
    ...
    def forward(self, z, bbox, z_im=None, y=None, masked_images=None):
        ...
        w = self.mapping(latent_vector.view(b * obj, -1))
        # preprocess bbox
        bmask = self.mask_regress(w, bbox)
...
class ResnetGenerator128_git_inpaint(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3):
        super(ResnetGenerator128_git_inpaint, self).__init__()
        self.num_classes = num_classes
    ...
    def forward(self, z, bbox, z_im=None, y=None, triples=None, masked_images=None):
        ...
        so = torch.cat([s, o], dim=0).cuda()  # Shape is (2*B, # of triples)  s[0] = y[0]

        label_embedding = self.label_embedding(so)  # [2B, # of triples, 180]
        # print(label_embedding)
        # print(so.shape, label_embedding.shape)
        ...
        # print(s_label_embedding, o_label_embedding)
        # print('[*] shape: {}, {}, {}'.format(s_label_embedding, pred_embedding.shape, o_label_embedding))
        ...
        # print(bbox)
        # print(y)
        # print('[*] shapes : {}, {}, {}'.format(w, bbox, y))
        # print(w.size(), bbox.size(), y.size())
        w = self.context(w, w, w, bbox, y)
        # print(w.shape)
        w = w.view(b * obj, -1)
        # print(w.shape)
...
class ResnetGenerator128_inpaint_triple(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3):
        super(ResnetGenerator128_inpaint_triple, self).__init__()
        self.num_classes = num_classes
        self.pred_classes = pred_classes
        ...
        self.res2 = ResBlock(ch * 16+4, ch * 8, upsample=True, num_w=num_w)
        self.res3 = ResBlock(ch * 8+4, ch * 4, upsample=True, num_w=num_w)
        self.res4 = ResBlock(ch * 4+4, ch * 2, upsample=True, num_w=num_w, psp_module=True)
        self.res5 = ResBlock(ch * 2+4, ch * 1, upsample=True, num_w=num_w, predict_mask=False)
        self.final = nn.Sequential(BatchNorm(ch),
                                   nn.ReLU(),
                                   conv2d(ch, output_dim, 3, 1, 1),
                                   nn.Tanh())
...
class ResnetGenerator128_gen_subject(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3):
        super(ResnetGenerator128_gen_subject, self).__init__()
        self.num_classes = num_classes
        self.pred_classes = pred_classes
...
class ResnetGenerator128_inpaint_subject(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3):
        super(ResnetGenerator128_inpaint_subject, self).__init__()
        self.num_classes = num_classes
        self.pred_classes = pred_classes
...
class ResnetGenerator128_git_inpaint_re(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3, object_embedding_size=180):
        super(ResnetGenerator128_git_inpaint_re, self).__init__()
        self.num_classes = num_classes

        self.label_embedding = nn.Embedding(num_classes, object_embedding_size)
        self.pred_embedding = nn.Embedding(pred_classes, object_embedding_size)
    ...
    def forward(self, z, bbox, z_im=None, y=None, triples=None, masked_images=None):
        ...
        pred_embedding = self.pred_embedding(p)  # [B, # of triples, 180]
        s_label_embedding, o_label_embedding = label_embedding.chunk(2, dim=0)  # [B, # of triples, 180]
        z_s, z_p, z_o = z.chunk(3, dim=0)
        z_s, z_p, z_o = z_s.squeeze(0), z_p.squeeze(0), z_o.squeeze(0)

        s_label_embedding = torch.cat((s_label_embedding, z_s), dim=-1)
...
class ResnetGenerator128_inpaint_triple_v2(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3, object_embedding_size=180):
        super(ResnetGenerator128_inpaint_triple_v2, self).__init__()
        self.num_classes = num_classes
    ...
    def forward(self, content):
        ...
        z_s, z_p, z_o = z.chunk(3, dim=1)
        z_s, z_p, z_o = z_s.squeeze(1), z_p.squeeze(1), z_o.squeeze(1)
        s_label_embedding = torch.cat((s_label_embedding, z_s), dim=-1)
        pred_embedding = torch.cat((pred_embedding, z_p), dim=-1)
        o_label_embedding = torch.cat((o_label_embedding, z_o), dim=-1)
...
class ResnetGenerator128_inpaint_triple_v3(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3, object_embedding_size=180):
        super(ResnetGenerator128_inpaint_triple_v3, self).__init__()
        self.num_classes = num_classes
        ...
        # self.context = BoxMultiHeadedAttention(1, num_w, dropout=0.0)
        self.context = MultiHeadAttention(1, num_w, num_w, num_w, dropout=0.0)
        # self.context = SemanticTripleAttentionModule(1, num_w, num_w, num_w, dropout=0.0)
        self.fc = nn.utils.spectral_norm(nn.Linear(z_dim, 4 * 4 * 16 * ch))
        self.res1 = ResBlock(ch * 16 + 4, ch * 16, upsample=True, num_w=num_w)
        self.res2 = ResBlock(ch * 16 + 4, ch * 8, upsample=True, num_w=num_w)
        self.res3 = ResBlock(ch * 8 + 4, ch * 4, upsample=True, num_w=num_w)
...
class ResnetGenerator128_inpaint_gcn(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3, object_embedding_size=180):
        super(ResnetGenerator128_inpaint_gcn, self).__init__()
        self.num_classes = num_classes
        ...
        # self.context = BoxMultiHeadedAttention(1, num_w, dropout=0.0)
        self.context = MultiHeadAttention(1, num_w, num_w, num_w, dropout=0.0)
        self.gcn = BatchGraphTripleConv(num_w, num_w, pooling='avg', mlp_normalization='none')
        self.fc = nn.utils.spectral_norm(nn.Linear(z_dim, 4 * 4 * 16 * ch))
        self.res1 = ResBlock(ch * 16 + 4, ch * 16, upsample=True, num_w=num_w)
        self.res2 = ResBlock(ch * 16 + 4, ch * 8, upsample=True, num_w=num_w)
        self.res3 = ResBlock(ch * 8 + 4, ch * 4, upsample=True, num_w=num_w)
    ...
    def forward(self, z, bbox, z_im=None, y=None, triples=None, masked_images=None):
        ...
        s_, o_ = torch.tensor(small_s), torch.tensor(small_o)  # [32, 8]
        so = torch.cat([s_, o_], dim=0).cuda()  # Shape is (2*B, # of triples)  s[0] = y[0]
        ...
        w, _ = self.gcn(s_label_embedding, pred_embedding, torch.stack((s, o), dim=-1))
        # w = self.context(o_label_embedding, pred_embedding, s_label_embedding)
        w = w.view(b * obj, -1)
        bmask = self.mask_regress(w, bbox)
...
class ResnetGenerator128_gen_gcn(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3, object_embedding_size=180):
        super(ResnetGenerator128_gen_gcn, self).__init__()
        self.num_classes = num_classes
...
class ResnetGenerator128_gen_triple(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3, object_embedding_size=180):
        super(ResnetGenerator128_gen_triple, self).__init__()
        self.num_classes = num_classes
...
class ResnetGenerator128_hvita(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3):
        super(ResnetGenerator128_hvita, self).__init__()
        self.num_classes = num_classes

        self.label_embedding = nn.Embedding(num_classes, 1024)
        # self.pred_embedding = nn.Embedding(pred_classes, 180)
...
class ResnetGenerator128_segment(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3):
        super(ResnetGenerator128_segment, self).__init__()
        self.num_classes = num_classes

        self.label_embedding = nn.Embedding(num_classes, 180)
        num_w = 180
        self.enc_conv1 = DupleConv(3+8, ch, ksize=7, pad=3, stride=2, num_w=num_w)  # 128x128  # coco 8
        self.enc_conv2 = DupleConv(ch, 2 * ch, stride=2, num_w=num_w)  # 64x64
        self.enc_conv3 = DupleConv(2* ch, 4 * ch, stride=2, num_w=num_w)  # 32x32
        self.enc_conv4 = DupleConv(4 * ch, 8 * ch, stride=2, num_w=num_w)  # 16x16

        self.res1 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 16x16
        self.res2 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 16x16
        self.res3 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 16x16
        self.res4 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 16x16
        self.res5 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 16x16
        self.res6 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 16x16
        self.res7 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 16x16
        self.res8 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 16x16
        self.res9 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 16x16

        self.dec_conv1 = DupleConv(8 * ch, 8 * ch, stride=1, num_w=num_w)  # 32*32
        self.dec_conv2 = DupleConv(8 * ch, 4 * ch, stride=1, num_w=num_w)  # 64*64
        self.dec_conv3 = DupleConv(4 * ch, 2 * ch, stride=1, num_w=num_w)  # 128*128
        self.dec_conv4 = DupleConv(2 * ch, 1 * ch, stride=1, num_w=num_w)  # 256*256
    ...
    def forward(self, z, bbox, z_im=None, y=None, triples=None, masked_images=None, segment=None):
        b, obj = z.size(0), z.size(1)
        # label_embedding = self.label_embedding(y)
        # segment [B,num_obj,H,W]  -> [B,embedding,H,W]
        x = torch.cat((masked_images, segment), 1)
        x = self.enc_conv1(x, is_duple=False)
        x = self.enc_conv2(x, is_duple=False)
        x = self.enc_conv3(x, is_duple=False)
        x = self.enc_conv4(x, is_duple=False)
        ...
        x = self.res7(x)
        x = self.res8(x)
        x = self.res9(x)  # [B, 512, 8, 8]

        x = F.interpolate(x, scale_factor=2, mode='nearest')  # [B, ch, h, w]
        x = self.dec_conv1(x, is_duple=False)
        x = F.interpolate(x, scale_factor=2, mode='nearest')
        x = self.dec_conv2(x, is_duple=False)
        x = F.interpolate(x, scale_factor=2, mode='nearest')
        x = self.dec_conv3(x, is_duple=False)
        x = F.interpolate(x, scale_factor=2, mode='nearest')
        x = self.dec_conv4(x, is_duple=False)
        x = self.final(x)
        return x
...
class ResnetGenerator128_edge(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, pred_classes=7, output_dim=3):
        super(ResnetGenerator128_edge, self).__init__()
        self.num_classes = num_classes

        self.label_embedding = nn.Embedding(num_classes, 180)
        num_w = 180
        self.enc_conv1 = DupleConv(3+1, ch, ksize=7, pad=3, stride=2, num_w=num_w)  # 128x128
        self.enc_conv2 = DupleConv(ch, 2 * ch, stride=2, num_w=num_w)  # 64x64
        self.enc_conv3 = DupleConv(2* ch, 4 * ch, stride=2, num_w=num_w)  # 32x32
        self.enc_conv4 = DupleConv(4 * ch, 8 * ch, stride=2, num_w=num_w)  # 16x16
    ...
    def forward(self, z, bbox, z_im=None, y=None, triples=None, masked_images=None, edge=None):
        b, obj = z.size(0), z.size(1)
        # label_embedding = self.label_embedding(y)
        # segment [B,num_obj,H,W]  -> [B,embedding,H,W]
        x = torch.cat((masked_images, edge), 1)
        x = self.enc_conv1(x, is_duple=False)
        x = self.enc_conv2(x, is_duple=False)
        x = self.enc_conv3(x, is_duple=False)
        x = self.enc_conv4(x, is_duple=False)
...
class GANformer(nn.Module):
    ...
    def forward(self, z, bbox, z_im=None, y=None, triples=None, masked_images=None):
        ...
        label_embedding = label_embedding.view(b * obj, -1)
        latent_vector = torch.cat((z, label_embedding), dim=1).view(b, obj, -1)
        latent_vector = self.graph_encoder(latent_vector, )  # add param.
        # bmask = self.mask_regress(w, bbox)
        ...
        # 4x4
        x = self.fc(z_im).view(b, -1, 4, 4)
        x = self.generator(x, latent_vector, masked_images)
        return x
...
class DupleConv(nn.Module):
    ...
    def forward(self, in_feat, is_duple=True):
        if is_duple:
            out_feat = self.duple(in_feat)
        else:
            out_feat = self.once(in_feat)
        return out_feat
