...
class DeepFillv2(nn.Module):
    def __init__(self, cnum_in=5, cnum=48, return_flow=False, checkpoint=None):
        super().__init__()
        self.generator = Generator(cnum_in=cnum_in, cnum=cnum, return_flow=return_flow)
        self.return_flow = return_flow
        if checkpoint is not None:
            generator_state_dict = torch.load(checkpoint)['G']
            self.load_state_dict(generator_state_dict, strict=True)
    ...
    def forward(self, content):
        x = content['image_contents']
        mask = content['mask']
        ones_x = torch.ones_like(x)[:, 0:1, :, :]  # sketch channel
        x = torch.cat([x, ones_x, ones_x*mask],
                      dim=1)  # concatenate channels
        if self.return_flow:
            x_stage1, x_stage2, offset_flow = self.generator(x, mask)
            return {'image_contents': x_stage2, 'x_stage1': x_stage1, 'offset_flow': offset_flow}
        else:
            x_stage1, x_stage2 = self.generator(x, mask)
            return {'image_contents': x_stage2, 'x_stage1': x_stage1}
    ...
    def infer(self,
        ...
        x = torch.cat([image_masked, ones_x, ones_x*mask],
                      dim=1)  # concatenate channels
        content = {'image_contents': x, 'mask': mask}
        if self.return_flow:
            fake_images_dict = self.forward(content)
        else:
            fake_images_dict = self.forward(content)

        x_stage1 = fake_images_dict['x_stage1']
        x_stage2 = fake_images_dict['image_contents']
        offset_flow = fake_images_dict['offset_flow']
