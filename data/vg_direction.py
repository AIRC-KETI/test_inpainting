...
class VgSceneGraphDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for cur in real_objs:
            ...
            d = [sx1-sx0-(ox1-ox0), sy1-sy0-(oy1-oy0)]
            ...
            '''
            ss = self.vocab['object_idx_to_name'][s]
            pp = self.vocab['pred_idx_to_name'][p]
            oo = self.vocab['object_idx_to_name'][o]
        ...
        obj_in_image = self.vocab['object_name_to_idx']['__image__']
        O = objs.size(0)
        for i in range(O - 1):
            triples.append([i, in_image, O-1])
        
        #
        triples = triples[:self.max_objects+1]
        triples = torch.LongTensor(triples)
        # texts = ''.join(texts)
        # print(O, image.shape, image.mean(), objs.shape, boxes.shape, triples.shape)
        # print(texts)
        # print(triples)
        # print("objs: ", objs, objs.size())
        # print("boxes: ", boxes, boxes.size())
        # print("triples: ", triples, triples.size())
        # print(triples)
        # print(boxes)
