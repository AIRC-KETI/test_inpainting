...
class VgSceneGraphDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(O-1,self.max_objects+2):
            triples.append([O - 1, in_image, O - 1])
        #
        triples = triples[:self.max_objects+1]
        triples = torch.LongTensor(triples)
        # print(O, image.shape, objs.shape, boxes.shape, triples.shape)
        # print(objs)
        # print(triples)
        # print(boxes)
        return image, objs, boxes, triples  #, texts
