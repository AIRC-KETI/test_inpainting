...
def main(args):
    ...
    object_height, object_width = [0]*200, [0]*200
    ...
    for epoch in range(args.start_epoch, args.total_epoch):
        ...
        with torch.no_grad():
            for idx, data in enumerate(tqdm(dataloader)):
                ...
                for j in range(label.size(-1)):  # in obj
                    ...
                    fake_images = fake_images_dict['image_contents'] * mask + masked_images * (1.-mask)  # [given an image, do all bbox]
                    '''
                ...
                # metric check
                    for i in range(label.size(0)):
                        ...
                        object_height[label[i,j].item()] = object_height[label[i,j].item()] + bbox[i,j,3].item()
                        object_width[label[i,j].item()] = object_width[label[i,j].item()] + bbox[i,j,1].item()

                        x1, y1, x2, y2 = int(real_images.size(3) * bbox[i,j,0]), int(real_images.size(2) * bbox[i,j,1]), int(real_images.size(3) * (bbox[i,j,2]+bbox[i,j,0])), int(real_images.size(2) * (bbox[i,j,3]+bbox[i,j,1]))  # [B, 1]
                        x1, y1, x2, y2 = min(max(0, x1), 254), min(max(0, y1), 254), min(max(1, x2), 255), min(max(1, y2), 255)
                        real_images_patch = torch.unsqueeze(real_images[i, :, y1:y2, x1:x2], 0)  # [NCHW]
                        fake_images_patch = torch.unsqueeze(fake_images[i, :, y1:y2, x1:x2], 0)  # [NCHW]
                        obj_real_images = F.interpolate(real_images_patch, size=(256, 256), mode='nearest')
                        obj_fake_images = F.interpolate(fake_images_patch, size=(256, 256), mode='nearest')
                        test_l1[label[i,j].item()] = test_l1[label[i,j].item()] + torch.mean(torch.abs(obj_real_images-obj_fake_images))
                        test_l2[label[i,j].item()] = test_l2[label[i,j].item()] + torch.mean(torch.square(obj_real_images-obj_fake_images))
                        test_ssim[label[i,j].item()] = test_ssim[label[i,j].item()] + ssim(obj_real_images, obj_fake_images)
                        test_psnr[label[i,j].item()] = test_psnr[label[i,j].item()] + psnr(obj_real_images, obj_fake_images)
                        test_lpips[label[i,j].item()] = test_lpips[label[i,j].item()] + lpips(obj_real_images, obj_fake_images)

                        obj_real_images = 2. * F.interpolate(real_images_patch, size=(299, 299), mode='nearest') - 1.
                        obj_fake_images = 2. * F.interpolate(fake_images_patch, size=(299, 299), mode='nearest') - 1.

                        temp_real_feats, temp_real_feats_1000 = inception_v3(obj_real_images)
                        ...
                        temp_fake_feats, temp_fake_feats_1000 = inception_v3(obj_fake_images)
    ...
    for i in range(len(test_is)):
        test_is_[i] = inception_score(fake_feats_1000[i][1:5001]) if fake_feats_1000[i].size(0) > 1 else 0
        test_fid[i] = compute_metric(real_feats[i][1:], fake_feats[i][1:]) if real_feats[i].size(0) > 2 else 0

    for i in range(len(test_l1)):
        print('[*] categories: {:03d}, count: {}'.format(i, count[i]))
        print('[*] object_width: {:03d}'.format(int(object_width[i]/(count[i]+1.e-6))))
        print('[*] object_height: {:03d}'.format(int(object_height[i]/(count[i]+1.e-6))))
    ...
    f= open(args.out_path + "/scenes_quantitative_results.txt","w+")
    for i in range(len(test_l1)):
        f.write('[*] categories: {:03d}, count: {:03d}\n'.format(i, count[i]))
        f.write('[*] object_width: {:03d}'.format(int(object_width[i]/(count[i]+1.e-6))))
        f.write('[*] object_height: {:03d}'.format(int(object_height[i]/(count[i]+1.e-6))))
